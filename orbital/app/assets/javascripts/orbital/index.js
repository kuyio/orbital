...
let application;
if (window.Stimulus) application = window.Stimulus;
else {
    application = (0, _stimulus.Application).start();
    application.debug = false;
    window.Stimulus = application;
}
application.register("orbital-avatar", (0, _avatarControllerDefault.default));
application.register("o-accordion", (0, _accordionControllerDefault.default));
application.register("orbital-modal", (0, _modalControllerDefault.default));
...
},{"@hotwired/stimulus":"hVNih","@parcel/transformer-js/src/esmodule-helpers.js":"5oERU"}],"cr6MB":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _stimulus = require("@hotwired/stimulus");
exports.default = class extends (0, _stimulus.Controller) {
    ...
    connect() {
        ...
        this._scrollY = 0;
        if (this.openValue) requestAnimationFrame(()=>this.show());
    }
    disconnect() {
    ...
    show() {
        ...
        this.element.dispatchEvent(new CustomEvent("orbital:modal:open", {
            bubbles: true
        }));
    }
    close() {
        ...
        this.element.dispatchEvent(new CustomEvent("orbital:modal:close", {
            bubbles: true
        }));
        this._clearTurboFrame();
    }
    backdropClick(event) {
        if (event.target === event.currentTarget && this.dismissibleValue) this.close();
    }
    handleKeydown(event) {
        if (event.key === "Escape") {
            event.preventDefault();
    ...
    _focusableElements() {
        ...
        return [
            ...this.panelTarget.querySelectorAll(selector)
...
exports.default = class extends (0, _stimulus.Controller) {
    ...
    setupHoverTrigger() {
        ...
        this.boundDelayedShow = ()=>{
            this.cancelShowTimeout();
            if (this.showDelayValue === 0) this.show();
            else this.showTimeout = setTimeout(()=>this.show(), this.showDelayValue);
        };
    ...
    handleClick(event) {
        ...
        if (this.triggerTarget.closest('[data-disabled="true"]') || this.triggerTarget.querySelector('[data-disabled="true"], [disabled], [aria-disabled="true"]')) return;
        event.preventDefault();
        this.toggle();
    }
...
function shouldAddVisualOffsets(element, isFixed, floatingOffsetParent) {
    ...
    return !!floatingOffsetParent && isFixed && floatingOffsetParent === (0, _dom.getWindow)(element);
}
function getBoundingClientRect(element, includeScale, isFixedStrategy, offsetParent) {
    ...
    if (domElement && offsetParent) {
        ...
        const offsetWin = (0, _dom.isElement)(offsetParent) ? (0, _dom.getWindow)(offsetParent) : offsetParent;
        let currentWin = win;
...
function getDocumentRect(html) {
    const scroll = (0, _dom.getNodeScroll)(html);
...
function getViewportRect(element, strategy, rootBoundary) {
    ...
    if (visualViewport) {
        ...
        // visual viewport.
        const layoutRelativeClientCoords = !(0, _dom.isWebKit)() || strategy === 'fixed';
        if (isLayoutViewport) {
...
function getClippingElementAncestors(element, cache) {
    ...
    while((0, _dom.isElement)(currentNode) && !(0, _dom.isLastTraversableNode)(currentNode)){
        ...
        else // The kept node carries the chain position for the next iteration.
...
function getClippingRect(_ref) {
    ...
    for(let i = 1; i < clippingAncestors.length; i++){
        ...
        top = (0, _utils.max)(rect.top, top);
        right = (0, _utils.min)(rect.right, right);
        bottom = (0, _utils.min)(rect.bottom, bottom);
        left = (0, _utils.max)(rect.left, left);
    }
    return {
        width: right - left,
        height: bottom - top,
    ...
    function refresh(skip, threshold) {
        ...
        function handleObserve(entries) {
            ...
            if (ratio !== threshold) {
                ...
                if (!ratio) // If the reference is clipped in place, the ratio is 0. Throttle
 ...
 */ function autoUpdate(reference, floating, update, options) {
    ...
    const ancestors = ancestorScroll || ancestorResize ? [
        ...
        ...floating ? (0, _dom.getOverflowAncestors)(floating) : []
    ] : [];
 ...
 */ const arrow = (options)=>({
        ...
        async fn (state) {
            ...
            const offset = (0, _utils.clamp)(minPadding, center, max);
            // If the reference is small enough that the arrow's padding causes it to
            // to point to nothing for an aligned placement, adjust the offset of the
            // floating element itself. To ensure `shift()` continues to take action,
            // a single reset is performed when this is true.
            const shouldAddOffset = !middlewareData.arrow && (0, _utils.getAlignment)(placement) != null && center !== offset && rects.reference[length] / 2 - (center < minPadding ? minPadding : maxPadding) - arrowDimensions[length] / 2 < 0;
 ...
 */ const inline = function(options) {
    ...
    return {
        ...
        async fn (state) {
            ...
            if (!nativeClientRects.length) return {};
            const clientRects = getRectsByLine(nativeClientRects);
            const fallback = (0, _utils.rectToClientRect)(getBoundingRect(nativeClientRects));
            const paddingObject = (0, _utils.getPaddingObject)(padding);
            function getBoundingClientRect() {
                // There are two rects and they are disjoined.
                if (clientRects.length === 2 && (clientRects[0].left > clientRects[1].right || clientRects[1].left > clientRects[0].right) && x != null && y != null) // Find the first rect in which the point is fully inside.
 ...
 */ const shift = function(options) {
    ...
    return {
        ...
        async fn (state) {
            ...
            const clampCoord = (axis, coord)=>(0, _utils.clamp)(coord + overflow[axis === 'y' ? 'top' : 'left'], coord, coord - overflow[axis === 'y' ? 'bottom' : 'right']);
    ...
    return {
        ...
        async fn (state) {
            ...
            if (noShift && !alignment) {
                if (isYAxis) availableWidth = width - 2 * (0, _utils.max)(overflow.left, overflow.right);
                else availableHeight = height - 2 * (0, _utils.max)(overflow.top, overflow.bottom);
            }
            await apply({
                ...state,
                availableWidth,
