...
module Orbital
  module AttributeUtils
    ...
    # `aria: { describedby: "xyz" }` rather than `"aria-describedby" => "xyz"`
    ...
    refine Hash do
      ...
      #
      #   original.merge_html_attributes(incoming)
      #
      #   => {:class=>["red", "blue"], :data=>{:size=>"medium", :controller=>"reply comment"}}
      #
      def merge_html_attributes(custom, parents = [], mergeable_attributes: DEFAULT_MERGEABLE_ATTRIBUTES)
        return custom unless custom.is_a?(Hash)

        overrides = custom.deep_symbolize_keys
        originals = deep_symbolize_keys

        originals.each_with_object(originals) do |(key, value), merged|
          next unless overrides.key?(key)

          merged[key] =
            process_pair(key, value, parents: parents, overrides: overrides, mergeable_attributes: mergeable_attributes)
      ...
      # @example
      #   { class: "blue", title: nil, lang: "", aria: { describedby: [] } }.deep_tidy
      #
      #   => { class: "blue" }
      #
      def deep_tidy
      ...
      def tidy_array(array)
        ...
        array.filter_map { |v| tidy_value(v) }
      end

      def tidy_remaining(value)
        # Strip empty strings - Rails converts them to bare attributes
        # for example:
        #    tag.div(hidden: "", data: { turbo_stream: "" })
        # -> <div hidden data-turbo-stream></div>
      ...
      def combine_hash(originals, overrides, parents:, mergeable_attributes:)
        originals.merge_html_attributes(overrides, parents, mergeable_attributes: mergeable_attributes)
      end
